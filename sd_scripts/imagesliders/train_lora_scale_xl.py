    ...
    for i in pbar:
        with torch.no_grad():
            ...
            try:
                ...
                flush()
                print(f'Error Occurred!: {np.array(img1).shape} {np.array(img2).shape}')
                continue
            # with network: の外では空のLoRAのみが有効になる
...
def main(args):
    ...
    if args.stylecheck is not None:
        ...
        for i in range(int(check[0]), int(check[1])):
            ...
            train(config=config, prompts=prompts, device=device, folder_main=folder_main, folders=folders,
                  scales=scales)
    else:
        train(config=config, prompts=prompts, device=device, folder_main=args.folder_main, folders=folders,
              scales=scales)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="attritbutes to disentangle (comma separated string)",
    )
    parser.add_argument(
        "--folder_main",
        type=str,
